...
export default function PostProjectForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate("/admin/ProjectExplore"); // Redirect to the project list
    } catch (error) {
  ...
  return (
    <div className="w-full">
      <div className="mb-4">
        <h2 className="text-3xl font-black text-brand-burgundy tracking-tight">Post Project</h2>
        <p className="text-gray-500 mt-1">Add a new project to the platform as an Administrator.</p>
      </div>
      ...
      <div className="w-full bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="flex flex-wrap justify-between items-center mb-6 gap-y-4 border-b border-gray-100 pb-4">
          {[
            { label: "Project Details", icon: <Home className="w-5 h-5 inline mr-2" /> },
            ...
            <span
              key={index}
              className={`text-sm sm:text-base flex items-center ${step === index + 1 ? "font-bold text-brand-gold" : "text-gray-400"
                }`}
            >
        ...
        {/* Step Content */}
        <div className="py-2">
          {step === 1 && <PropertyDetails formData={formData} setFormData={setFormData} cities={[]} />}
          ...
          {step === 4 && (
            <div className="max-w-3xl mx-auto">
              <h2 className="text-lg sm:text-xl font-bold mb-4">Upload Project Images</h2>
        ...
        <div className="flex justify-between mt-8 pt-4 border-t border-gray-100">
          ...
          {step < 4 ? (
            ...
            <Button onClick={handleSubmit} text="Submit Project" />
          )}
        </div>
      </div>
    </div>
